...
class plgEditorCKeditor extends JPlugin
{
	...
	function onDisplay( $name, $content, $width, $height, $col, $row, $buttons = true, $id = null, $asset = null, $author = null)
	{
		...
		if ($style_file != '')
		{
			if (file_exists(dirname(__FILE__).DS.'styles'.DS.$style_file))
			{
				$editor .= ",stylesCombo_stylesSet  : '".str_replace('.js','',$style_file).":".JURI::root()."plugins/editors/ckeditor/styles/".$style_file."'";
				$editor .= ",stylesSet  : '".str_replace('.js','',$style_file).":".JURI::root()."plugins/editors/ckeditor/styles/".$style_file."'";
				$style = true;
			}
		}
		$template = false;
		...
		if ($template_file != '')
		{
			if (file_exists(dirname(__FILE__).DS."templates".DS.$template_file))
			{
				$editor .= ",templates_files  :  ['".JURI::root()."/plugins/editors/ckeditor/templates/".$template_file."']";
				$editor .= ",templates : '".str_replace('.js', '', $template_file)."'";
				$template = true;
			}
		}
		...
		if ($css_files != '')
		{
			...
			{
				if (file_exists(dirname(__FILE__).DS.'css'.DS.trim($file)))
				{
